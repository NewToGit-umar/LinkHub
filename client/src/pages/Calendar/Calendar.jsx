        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {cells.map((cell) => {
            ...
            return (
              <div
                key={key}
                className={`min-h-[90px] p-2 border rounded ${isCurrentMonth ? 'bg-white' : 'bg-gray-50'} cursor-pointer`}
                ...
                {posts.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {posts.slice(0,3).map(p => (
                      <div key={p._id} className="text-xs p-1 bg-blue-50 rounded text-blue-700">{p.content.slice(0,60)}{p.content.length>60?'…':''}</div>
                    ))}
                    {posts.length > 3 && <div className="text-xs text-gray-500">+{posts.length-3} more</div>}
                  </div>
                )}
              </div>
            )
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-3xl bg-white rounded p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Scheduled for {selectedDatePosts.date}</h3>
              <button onClick={() => setSelectedDatePosts(null)} className="text-gray-500">Close</button>
            </div>
            <div className="space-y-4 max-h-[60vh] overflow-auto">
              {selectedDatePosts.posts.length === 0 && <div className="text-gray-600">No scheduled posts for this date.</div>}
              {selectedDatePosts.posts.map(p => (
                <div key={p._id} className="p-3 border rounded bg-white">
                  ...
                  <div className="text-xs text-gray-500">Platforms: {p.platforms.join(', ')}</div>
                  <div className="text-xs text-gray-400">Status: {p.status}</div>
                </div>
              ))}
            </div>
            <div className="flex justify-end mt-4">
              <button onClick={() => setSelectedDatePosts(null)} className="px-4 py-2 border rounded">Close</button>
            </div>
          </div>
        </div>
      )}
